$(function(){
	var min_y , max_y , picYear , picMonth;
	var date = new Date();
	var d = date.getDate();
	var m = date.getMonth();
	...
	function nowDate(){
		...
		if($('.day_group div').length == 0){
			$('.btn_group button').addClass('btn_dis');
			$('.btn_group .create_check').removeClass('btn_dis');
		}
		})
	...
	function getDayOfWeek(firstDay){
		// var className = "m_l_"+firstDayOfWeek;
		if(firstDay == 7){
			// $('.day_group div').eq(0).removeClass('m_l_1 m_l_2 m_l_3 m_l_4 m_l_5 m_l_6');
		}else{
			// $('.day_group div').eq(0).removeClass('m_l_1 m_l_2 m_l_3 m_l_4 m_l_5 m_l_6').addClass(className);
...
/////////////////////////////////////各式按鈕///////////////////////////////////////
	...
	$('.btn_group .edit_check').on('click',function(){
		...
		$('.btn_left,.btn_right').unbind('click');
	})
	// "送出按鈕"的功能
...
/////////////////////////////////////Ajax部份///////////////////////////////////////
	...
	$('.create_check').on('click',function(){
		...
		$.ajax({
			url: '/supportcase/case_table_save',
			type:'POST',
			cache:false,
			datatype:'json',
	...
	$('.reset_check').on('click',function(){
		var token = $("input[name='_token']").val();
		var a = [];
		var y = parseFloat($('.day_group').find('div').data('year'));
		var m = parseFloat($('.day_group').find('div').data('month'));
